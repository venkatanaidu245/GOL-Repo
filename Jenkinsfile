...
//}
  ...
  stage('Sonarqube') {
    ...
    stage('Deploy War') {
      steps {
          //deploy adapters: [tomcat8(credentialsId: 'tomcat-cred', path: '', url: 'http://18.220.224.44:8080/')], contextPath: null, war: '**/*.war'
...
post {
        ...
        failure {
            mail to:"rayankulavenkatanaidu@gmail.com", subject:"FAILURE: ${currentBuild.fullDisplayName}", body: "Build failed"
        }
    }       
}
